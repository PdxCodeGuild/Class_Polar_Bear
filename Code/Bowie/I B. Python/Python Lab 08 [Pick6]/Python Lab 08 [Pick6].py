...
# newN-related code to deal with replacement of numbers
# idea: import itertools for permutations, number generators, etc.
...
def randomSixDigits():
    ...
    for number in range(6):
        while True:
            newN = random.randint(1,99)
            if newN not in _list:
                _list.append(newN)
                break
    return _list

def matchN(winningNumbers, playerNumbers):
    count = 0
