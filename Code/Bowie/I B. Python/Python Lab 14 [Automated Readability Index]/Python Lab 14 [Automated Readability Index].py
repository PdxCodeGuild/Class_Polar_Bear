...
def ARI(text):
    char_c = countCharacters(text)
    word_c = countWords(text)
    sent_c = countSentences(text)

    punc = 4.71*(char_c/word_c) + 0.5*(word_c/sent_c) - 21.43

    if punc >= 14:
        return 14
    else:
        return math.ceil(punc)

print(f"punc = {ARI('hello world')}")
